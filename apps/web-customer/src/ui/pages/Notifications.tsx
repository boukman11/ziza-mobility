...
export default function Notifications() {
  ...
  async function load() {
    ...
    try {
      const res = await apiFetch<any>("/v1/customer/notifications?limit=50&offset=0", { method: "GET" });
      setItems(res.items || []);
  ...
  async function markRead(id: string) {
    try {
      await apiFetch<any>(`/v1/customer/notifications/${id}/read`, { method: "POST" });
      await load();
    } catch (e: any) { setErr(e.message);
      toast.push(e.message, "error"); }
  }
  ...
  return (
    <div className="card">
      ...
      <table>
        <thead>
          <tr><th>Status</th><th>Title</th><th>When</th><th></th></tr>
        </thead>
        <tbody>
          {items.map(n => (
            <tr key={n.id}>
