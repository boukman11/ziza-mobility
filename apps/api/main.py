...
def safe_notify_user(db: Session, user_id, notif_type: str, title: str, body: str, meta: dict | None = None):
    """Best-effort notification wrapper that never breaks core business flows."""
    try:
        notify_user(db, user_id, notif_type, title, body, meta)
    except Exception:
        db.rollback()
        logger.exception("notify_user_failed user_id=%s notif_type=%s", user_id, notif_type)
...
def driver_accept(trip_id:str, claims=Depends(driver_auth), db: Session=Depends(get_db)):
    ...
    safe_notify_user(db, t.customer_user_id, "TRIP_ASSIGNED", "Driver assigned", f"Driver accepted trip {t.id}", {"tripId": str(t.id), "driverUserId": str(u.id)})
    safe_notify_user(db, u.id, "TRIP_ASSIGNED", "Trip assigned", f"You accepted trip {t.id}", {"tripId": str(t.id)})
